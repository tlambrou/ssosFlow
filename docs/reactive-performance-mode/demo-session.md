...
Use note 44 to inspect the currently loaded action document, latest slot execution, first eight action-bank rows, first eight macro-bank rows, and first eight state-bank rows. Use note 45 after editing `reactive-actions.txt`.
...
- The status panel reports slot 0, a successful last execution, shows the refreshed next-action preview, marks the corresponding action-bank row, and shows macro/state values from the loaded action document.
...
- The status panel is a compact diagnostic dialog with the first reusable action-bank, macro-bank, state-bank, and next-action preview read models, not the final Cue-page performance panel.
