...
Issue: #63, #123, #154, #162, #166, #170, #184, #188, #190, #192, #194, #196, #198, #210.

This guide sets up the current Reactive Performance Mode vertical slice: the bundled demo-session template, eight Generic MIDI pad actions, session-local action loading, seeded marker-trigger and region-trigger demo landmarks, first Cue-page trigger clips with simple rhythm notes and harmony/macro triads, scene-trigger demo actions, action/macro/state/harmony/session-clock/track/routing/trigger-slot/MIDI-input status read models, status-panel trigger controls, cached Generic MIDI controller feedback, and the Reactive Rhythm State MVP LuaProc insertion path.
...
- The status panel is a compact diagnostic dialog with first reusable control, action-bank, macro-bank, state-bank, harmony-bank, next-action command preview, queued-action command details, MIDI-input read model, session-clock read model, track-state read model, mixer scene-state read model, routing read model, and bounded trigger-slot read model.
