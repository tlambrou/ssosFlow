...
Issue: #63, #123, #154, #162, #166, #170, #184, #188, #190, #192, #194.

This guide sets up the current Reactive Performance Mode vertical slice: the bundled demo-session template, eight Generic MIDI pad actions, session-local action loading, seeded marker-trigger and region-trigger demo landmarks, first Cue-page trigger clips with simple rhythm notes and harmony/macro triads, scene-trigger demo actions, action/macro/state/harmony/routing/trigger-slot status read models, status-panel trigger controls, cached Generic MIDI controller feedback, and the Reactive Rhythm State MVP LuaProc insertion path.
...
Use note 44 to inspect whether Reactive Performance Mode is enabled, the currently loaded action document, latest slot execution, first eight performance controls, first eight action-bank rows, first eight macro-bank rows, first eight state-bank rows, and first eight harmony-bank rows. The status panel also includes resolved next-action command details, queued-action command details for pending quantized actions, routing state, trigger-slot state, eight large slot-trigger buttons, and an Enable/Disable Mode button for controller-parity checks during setup. The Cue page also shows the dedicated Reactive Performance panel above the trigger strip grid; its eight slot buttons reflect loaded action names, its compact summary includes the next command preview, queued command details, routing state, and the first trigger-slot rows, and its controls disable themselves when a slot is unavailable or Reactive Performance Mode is disarmed. Use note 45 after editing `reactive-actions.txt`. Use note 47 to arm or disarm Reactive Performance Mode from the controller; when disabled, performance actions report a disabled status and do not launch cues, mutate rhythm parameters, or update macro/state/harmony values.
...
- The status panel shows eight slot-trigger buttons, visibly disables empty/unavailable controls, has an Enable/Disable Mode button, and lists Trigger Slots with populated seeded clips such as `Reactive Cue 0 Reset` plus empty slots.
...
- The status panel is a compact diagnostic dialog with first reusable control, action-bank, macro-bank, state-bank, harmony-bank, next-action command preview, queued-action command details, routing read model, and bounded trigger-slot read model.
