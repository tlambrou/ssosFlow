...
- The status panel reports slot 0, a successful last execution, marks the corresponding action-bank row, and shows macro/state values from the loaded action document.
...
- The status panel is a compact diagnostic dialog with the first reusable action-bank, macro-bank, and state-bank read models, not the final Cue-page performance panel.
