...
Issue: #63, #123, #154, #162, #166, #170, #184, #188, #190, #192, #194, #196, #198, #210, #212.

This guide sets up the current Reactive Performance Mode vertical slice: the bundled demo-session template, eight Generic MIDI pad actions, session-local action loading, seeded marker-trigger and region-trigger demo landmarks, first Cue-page trigger clips with simple rhythm notes and harmony/macro triads, scene-trigger demo actions, action/macro/Macro Snapshot/state/harmony/session-clock/track/routing/trigger-slot/MIDI-input status read models, status-panel trigger controls, cached Generic MIDI controller feedback, and the Reactive Rhythm State MVP LuaProc insertion path.
...
The template also installs the demo `reactive-actions.txt` next to the new `.ardour` session file when Lua file I/O is available. It is the repeatable MVP setup path, but it is not a full `.ardour` session archive with a polished arrangement, ports, and environment-specific connections.
...
The template also seeds the first small named MIDI TriggerBox regions so the Cue page opens with visible, launchable MIDI clips instead of empty lanes. The helpers are idempotent by slot: they leave any existing trigger-region content untouched and only fill empty slots.

| Lane | Slot | Seeded trigger region | MIDI notes |
| --- | --- | --- | --- |
| `Reactive Rhythm Lane` | 0 | `Reactive Cue 0 Reset` | 36 |
| `Reactive Rhythm Lane` | 1 | `Reactive Cue 1 Tighten` | 38 |
| `Reactive Rhythm Lane` | 2 | `Reactive Cue 2 Sparse` | 41 |
| `Reactive Harmony Lane` | 0 | `Reactive Harmony i` | 48, 51, 55 |
| `Reactive Macro Lane` | 0 | `Reactive Macro Filter` | 60, 64, 67 |
| `Reactive Macro Lane` | 1 | `Reactive Macro Texture` | 67, 72, 74 |
...
Use note 44 to inspect whether Reactive Performance Mode is enabled, the currently loaded action document, latest slot execution, latest MIDI Input state, first eight performance controls, first eight action-bank rows, first eight macro-bank rows, stored Macro Snapshots, first eight state-bank rows, and first eight harmony-bank rows. The status panel also includes resolved next-action command details, queued-action command details for pending quantized actions, session-clock state, track state, scene state, routing state, trigger-slot state, eight large slot-trigger buttons, and an Enable/Disable Mode button for controller-parity checks during setup. The Cue page also shows the dedicated Reactive Performance panel above the trigger strip grid; its eight slot buttons reflect loaded action names, its compact summary includes the next command preview, queued command details, MIDI Input state, Macro Snapshot state, session-clock state, track state, scene state, routing state, and the first trigger-slot rows, and its controls disable themselves when a slot is unavailable or Reactive Performance Mode is disarmed. Use note 45 after editing `reactive-actions.txt`. Use note 47 to arm or disarm Reactive Performance Mode from the controller; when disabled, performance actions report a disabled status and do not launch cues, mutate rhythm parameters, or update macro/state/harmony values.
...
- A template-created session contains Cue-page visible `Reactive Rhythm Lane`, `Reactive Harmony Lane`, `Reactive Macro Lane`, a visible `Breakdown` marker, a non-hidden `Breakdown Loop` region on the rhythm lane, and the six named trigger clips listed above; a manually created session has at least one MIDI route in controller-facing route slot 0.
...
- The status panel shows eight slot-trigger buttons, visibly disables empty/unavailable controls, has an Enable/Disable Mode button, lists the latest MIDI Input note/CC readout with channel, value or velocity, source, match count, and matched action when available, lists Macro Snapshots with stored macro counts and value previews after snapshot actions run, lists Session State with stopped/rolling transport plus speed, record, loop, locate, BBT, tempo, meter, sample position, route counts, lists Track State with active/mute/solo/record/gain values for controller-facing routes, lists Scene State with stored/empty mixer-scene validity, and lists Trigger Slots with populated seeded clips such as `Reactive Cue 0 Reset` plus empty slots.
...
- The status panel reports slot 0, a successful last execution, shows `MIDI Input: note ch=10 note=36 velocity=100 source=event` or `source=bytes` with a matched action count, shows the refreshed next-action preview with command details such as the next cue or macro command, shows Session State with current transport and musical-clock context, shows Track State for route 0, shows Scene State, shows route 0 with `Reactive Rhythm State MVP`, lists route 0 trigger-slot state with `Reactive Cue 0 Reset playable`, marks the corresponding action-bank row, and shows macro/state/harmony values from the loaded action document.
- When a nonzero-quantized action is pending, the status panel and Cue-page panel list the queued action's command details under the queued summary, using the already-resolved command values that will fire at release time.
...
- Move CC 22 on MIDI channel 1 and confirm route 0 rhythm density follows the controller value while the routing summary shows the density value and the MIDI Input row reports `cc ch=1 cc=22 value=<value> source=bytes matches=1` or `source=event`. The same CC also sets route 0 slot 0 trigger follow probability through `DO trigger probability 0 0 midi-value`. The Macro Snapshots summary should show stored texture snapshot targets, the macro bank should show `filter` tracking from `0.0` to `1.0` plus a macro morph between stored texture snapshots: `texture` moves from `0.20` to `0.80`, and `space` moves from `0.80` to `0.25`. The preview command details should reflect the current CC-derived trigger-probability and macro morph values.
...
- The status panel is a compact diagnostic dialog with first reusable control, action-bank, macro-bank, Macro Snapshot read model, state-bank, harmony-bank, next-action command preview, queued-action command details, MIDI-input read model, session-clock read model, track-state read model, mixer scene-state read model, routing read model, and bounded trigger-slot read model.
- The repo packages a repeatable `Reactive Performance MVP` SessionInit template plus a session-local action document under `examples/reactive-performance-mvp/`. The template installs the demo action document into new sessions without overwriting an existing `reactive-actions.txt` when Lua file I/O is available, seeds the `Breakdown` marker plus `Breakdown Loop` timeline region without duplicating existing visible landmarks, and fills the first six empty Cue-page trigger slots with named MIDI clips containing 12 total notes. This repo does not yet package a full `.ardour` demo session archive or polished generated arrangement.
