...
Issue: #63, #123, #154, #162, #166, #170, #184.

This guide sets up the current Reactive Performance Mode vertical slice: the bundled demo-session template, eight Generic MIDI pad actions, session-local action loading, seeded marker-trigger and region-trigger demo landmarks, first Cue-page trigger clip placeholders, scene-trigger demo actions, action/macro/state/harmony status read models, status-panel trigger controls, cached Generic MIDI controller feedback, and the Reactive Rhythm State MVP LuaProc insertion path.
...
The template also installs the demo `reactive-actions.txt` next to the new `.ardour` session file when Lua file I/O is available. It is the repeatable MVP setup path, but it is not a full `.ardour` session archive with generated musical note content, ports, and environment-specific connections.
...
The template also seeds the first small named MIDI TriggerBox regions so the Cue page opens with visible, launchable placeholders instead of empty lanes. The helper is idempotent by slot: it leaves any existing trigger-region content untouched and only fills empty slots.

| Lane | Slot | Seeded trigger region |
| --- | --- | --- |
| `Reactive Rhythm Lane` | 0 | `Reactive Cue 0 Reset` |
| `Reactive Rhythm Lane` | 1 | `Reactive Cue 1 Tighten` |
| `Reactive Rhythm Lane` | 2 | `Reactive Cue 2 Sparse` |
| `Reactive Harmony Lane` | 0 | `Reactive Harmony i` |
| `Reactive Macro Lane` | 0 | `Reactive Macro Filter` |
| `Reactive Macro Lane` | 1 | `Reactive Macro Texture` |
...
- A template-created session contains Cue-page visible `Reactive Rhythm Lane`, `Reactive Harmony Lane`, `Reactive Macro Lane`, a visible `Breakdown` marker, a non-hidden `Breakdown Loop` region on the rhythm lane, and the six named trigger clip placeholders listed above; a manually created session has at least one MIDI route in controller-facing route slot 0.
