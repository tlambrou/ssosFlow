...
- This script does not insert itself into routes. Explicit C++ helper insertion is covered by `ReactiveRhythmRouteInserter`, and declarative insertion can be requested with `DO rhythm insert <route-index>`; script presets, controller feedback, and demo-session routing remain follow-up work.
